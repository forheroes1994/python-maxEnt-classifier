...
class Classifier:
    ...
    #     feat_alphabet = self.creat_feature_alphabet(train_Inst)
    #     self.weight_array = self.Init_weight_array(train_Inst)
    #     # print(train_exam_list[0].word_index)
    #     steps, all_loss = 0, 0
    #     for epoch in range(1, self.hyperparameter_1.epochs + 1):
    #         steps += 1
    #         print("————第{}轮迭代，共{}轮————".format(epoch, self.hyperparameter_1.epochs))
    #         batchBlock = self.set_batchBlock(train_exam_list)
    #         # init_grad_w = np.zeros((len(feat_alphabet.list), self.hyperparameter_1.class_num))
    #         random.shuffle(train_exam_list)
    #         # sentence_result = self.Y_list(train_exam_list) # 一轮迭代更新一次
    #         # train_size = len(sentence_result)
    #         corrects, accuracy, sum,  all_loss = 0, 0, 0, 0
    #         for every_batchBlock in range(batchBlock):
    #             # steps += 1
    #             # print(every_batchBlock)
    #             exam = []
    #             # start_pos = every_batchBlock * self.hyperparameter_1.batch_size
    #             # end_pos = (every_batchBlock + 1) * self.hyperparameter_1.batch_size
    #             # if end_pos > len(train_exam_list):
    ...
    #             for idx in range(len(sentence_result)):
    #                 sum += 1
    #                 y = self.softmax(sentence_result[idx])
    #                 # print(y)
    #                 each_loss = self.count_loss(y)
    #                 all_loss += each_loss
    #                 pd_l_for_y = np.array(y) - np.array(train_exam_list[start_pos + idx].label_index)
    #                 # pd_l_for_y = pd_l_for_y / self.hyperparameter_1.batch_size
    #                 word_max_num = self.get_max_index(y)
    #                 label_num = train_exam_list[start_pos + idx].max_label_index
    #                 if word_max_num == label_num:
    #                     corrects += 1
    #                 for j in train_exam_list[start_pos + idx].word_index:
    #                     init_grad_w[j] += pd_l_for_y
    #                     self.weight_array[j] -= self.hyperparameter_1.lr * np.array(init_grad_w[j])
    #
    #             #         if j not in exam:
    #             #             exam.append(j)
    #             # for i in exam:
    #             #     self.weight_array[i] -= self.hyperparameter_1.lr * np.array(init_grad_w[i])
    #
    #         if steps % self.hyperparameter_1.log_interval == 0:
    #             accuracy = corrects / sum * 100.0
    #             sys.stdout.write(
    ...
    #     # sentence_result = self.Y_list(dev_exam_list)
    #     # train_size = len(sentence_result)
    #     # batchBlock = self.set_batchBlock(dev_exam_list)
    #     # for every_batchBlock in range(batchBlock):
    #     #     start_pos = every_batchBlock * self.hyperparameter_1.batch_size
    #     #     end_pos = (every_batchBlock + 1) * self.hyperparameter_1.batch_size
    #     #     if end_pos > len(dev_exam_list):
    #     #         end_pos = len(dev_exam_list)
    #     #     for idx in range(start_pos, end_pos):
    #     #         num = self.get_max_index(sentence_result[idx])
    #     #         label_num = dev_exam_list[idx].max_label_index
    #     #         if num == label_num:
